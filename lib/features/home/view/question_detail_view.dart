...
class _QuestionDetailViewState extends State<QuestionDetailView> {
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.postId == null) {
      return Scaffold(
        ...
        appBar: _buildAppBar(),
        body: _buildStatusMessage("Question not found."),
      );
    }
    ...
    return Scaffold(
      ...
      body: StreamBuilder<DocumentSnapshot>(
        stream: _db.collection('posts').doc(widget.postId).snapshots(),
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Container(
                          ...
                          child: Row(
                            children: [
                              _tabButton("Comments ($commentCount)", !isAiView, () => setState(() => isAiView = false)),
                              _tabButton("Ask Sesh", isAiView, () => setState(() => isAiView = true), icon: Icons.auto_awesome),
                            ],
                          ),
                        ),
                        const SizedBox(height: 25),
                        isAiView ? const AiTutorHelpView() : _buildCommentsList(widget.postId!),
                      ],
                    ),
                  ),
                ),
              ),
              if (!isAiView) _buildCommentInput(widget.postId!),
            ],
          );
        },
      ),
  ...
  Widget _buildCommentInput(String postId) {
    final bool canSend = _commentController.text.trim().isNotEmpty && !_isSubmittingComment;

    return Container(
      padding: EdgeInsets.fromLTRB(20, 10, 20, MediaQuery.of(context).padding.bottom + 10),
      decoration: BoxDecoration(
        color: const Color(0xFF1E243A),
        border: Border(top: BorderSide(color: Colors.white.withValues(alpha: 13))),
      ),
      child: Row(
        children: [
          Expanded(
          ...
          const SizedBox(width: 10),
          GestureDetector(
            onTap: canSend ? () => _submitComment(postId) : null,
            child: CircleAvatar(
              backgroundColor: canSend ? const Color(0xFF00C09E) : Colors.white24,
              child: _isSubmittingComment
                  ...
                  : Icon(Icons.send, color: canSend ? const Color(0xFF0F142B) : Colors.white54, size: 20),
            ),
          ),
        ],
      ),
