        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              _buildBulletPoint(
                'Upload or share harmful, abusive, sexual, or illegal content',
              ),
              ...
              _buildSectionTitle(
                '4. Educational Content, Notes, and User Submissions',
              ),
              _buildParagraph(
              ...
              _buildParagraph(
                'Tutors are independent users, not employees or agents of AutoXyrium.',
              ),
              _buildParagraph(
                'AutoXyrium does not guarantee academic results, performance, or outcomes.',
              ),
              _buildParagraph(
                'Tutors set their own rates. Payments may be facilitated through third-party payment providers.',
              ),
              _buildParagraph(
                'Unless explicitly stated, payments are non-refundable.',
              ),
              ...
              _buildBulletPoint(
                'AutoXyrium shall not be liable for indirect, incidental, or consequential damages',
              ),
              _buildBulletPoint(
                'Our total liability shall not exceed the amount paid (if any) by you to Seshly',
              ),

              _buildSectionTitle('10. Governing Law'),
              _buildParagraph(
                'This Agreement is governed by the laws of the Republic of South Africa, without regard to conflict-of-law principles.',
              ),
              ...
              _buildBulletPoint(
                'Age or date of birth (for eligibility verification)',
              ),
              _buildSubtitle('b. Usage Data'),
              _buildBulletPoint(
                'App interactions, session data, device information',
              ),
              _buildBulletPoint('Logs, crash reports, and performance metrics'),
              _buildSubtitle('c. Communications'),
              _buildBulletPoint(
                'Messages, support requests, and in-app interactions',
              ),
              ...
              _buildBulletPoint(
                'With service providers (e.g., hosting, payments, analytics)',
              ),
              _buildBulletPoint('To comply with legal requirements'),
              _buildBulletPoint(
                'To protect the rights and safety of users and AutoXyrium',
              ),
              ...
              _buildParagraph(
                'Parents or guardians may contact us to review or delete a child\'s data.',
              ),
              ...
              _buildParagraph(
                'Requests can be made through in-app support or official contact channels.',
              ),

              _buildSectionTitle('19. Contact Information'),
              _buildParagraph(
                'For questions or concerns regarding these Terms or Privacy Policy:',
              ),
...
class _LoginPageViewState extends State<LoginPageView> {
  ...
  bool _isInstantTutorLoading = false;
  String? _authError;
  ...
  Future<void> _handleInstantTutorModeTap() async {
    if (_isLoading || _isInstantTutorLoading) return;
    FocusManager.instance.primaryFocus?.unfocus();
    setState(() {
      _authError = null;
      _isInstantTutorLoading = true;
    });

    try {
      await _authService.enterInstantTutorMode();
      if (!mounted) return;
      final navigator = Navigator.of(context);
      debugPrint('Instant Tutor Mode button: auth call succeeded.');
      if (navigator.canPop()) {
        debugPrint(
          'Instant Tutor Mode button: closing login flow with popUntil(isFirst).',
        );
        navigator.popUntil((route) => route.isFirst);
      } else {
        debugPrint(
          'Instant Tutor Mode button: no route to pop, waiting for AuthWrapper rebuild.',
        );
      }
    } on FirebaseAuthException catch (error, stackTrace) {
      debugPrint(
        'Instant Tutor Mode button failed [${error.code}]: ${error.message}',
      );
      debugPrintStack(stackTrace: stackTrace);
      if (!mounted) return;
      await _showInstantTutorModeFailure(_instantTutorModeErrorMessage(error));
    } catch (error, stackTrace) {
      debugPrint('Instant Tutor Mode button failed: $error');
      debugPrintStack(stackTrace: stackTrace);
      if (!mounted) return;
      await _showInstantTutorModeFailure(
        'Instant Tutor Mode is unavailable right now. Please try again.',
      );
    } finally {
      if (mounted) {
        setState(() => _isInstantTutorLoading = false);
      }
    }
  }

  String _instantTutorModeErrorMessage(FirebaseAuthException error) {
    switch (error.code) {
      case 'operation-not-allowed':
        return 'Instant Tutor Mode is currently unavailable.';
      case 'network-request-failed':
        return 'Check your connection and try Instant Tutor Mode again.';
      case 'too-many-requests':
        return 'Too many attempts right now. Please wait a moment and retry.';
      default:
        return 'Instant Tutor Mode could not be opened right now.';
    }
  }

  Future<void> _showInstantTutorModeFailure(String message) async {
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              SizedBox(
                width: double.infinity,
                child: _buildInstantTutorModeCard(primaryColor: primaryColor),
              ),
        ...
        builder: (context, constraints) {
          ...
          final details = Column(
            ...
            children: [
              ...
              const SizedBox(height: 10),
              const Text(
                'Tutor discovery and booking only.',
          ...
          if (stacked) {
            return Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [details, const SizedBox(height: 14), cta],
            );
          }
